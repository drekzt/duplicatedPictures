        ...
        if not re.match('.*/\..*|^/dev.*|^/boot.*|^/opt.*|^/proc.*|^/run.*|^/tmp.*|^/usr.*|^/var.*|.*videothumbs|.*audiothumbs|.*Adobe Photoshop.*',root):
            for file in files:
                ...
                try: 
                    ...
                    print (' Mean {}. Dev {}. {}'.format(mean,sdev,file))
                except:
                    try:
...
def similarity(collection_name,factor):
    ...
    for nextitem in simornot:
        if (comparetriple(previtem,nextitem,SIMILARITYFIELD,factor) and comparetriple(previtem,nextitem,CONTRASIMFIELD,factor)):
